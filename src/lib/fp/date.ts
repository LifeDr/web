...
export const dateFormat = (format: string, date: DateTime): string =>
  (dateFormatMemo[format] = dateFormatMemo[format] || createDateFormat(format))(+dayjs(date))
...
export class DateTime extends Date {
  ...
  startOf(type: "day" | "week" | "month" | "year"): DateTime {
    ...
    switch (type) {
      ...
      case "week": {
        return date.add({days: -date.getDay()})
      }
      case "month": {
        return date.with({date: 1})
      }
      case "year": {
  ...
  endOf(type: "day" | "week" | "month" | "year") {
    ...
    switch (type) {
      case "day": {
        return date.startOf("day").add({days: +1, milliseconds: -1})
      }
      case "week": {
        return date.add({days: -date.getDay() + 7})
      }
      case "month": {
        return date.add({months: +1}).with({date: 1})
      }
    }
    return date
  }
